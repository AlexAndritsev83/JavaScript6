...
function collect(arr1) {
    checkForArray(arr1);
   
     const arr2 = arr1.reduce((acc, val) => Array.isArray(val) 
                                            ? acc.concat(collect(val)) : acc.concat(val), []);
     
     if (arr2.some(x => !(typeof x === 'number'))){
        
        return null;

      } else {
        
        return arr2.reduce((acc1, val1)=> acc1 + val1, 0);    
      }    
};
...
//Моя Проверка
// const array6 = {a:7, b:8 };
// console.log(collect(array6)); // 12

// const array7 = [[['String', 2], [1, 2]], [[2, 1], [1, 2]]];
// console.log(collect(array7)); // 12
